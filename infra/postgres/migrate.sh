...
run_prisma() {
  # Use node to execute prisma CLI directly to avoid workspace resolution issues
  if [ -f ./node_modules/prisma/build/index.js ]; then
    node ./node_modules/prisma/build/index.js "$@" --schema="$PRISMA_SCHEMA_PATH"
    return $?
  fi
  # Fallback to pnpm exec if available (though this may fail in certain environments)
