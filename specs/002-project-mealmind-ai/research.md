...
- **Rationale**: Meets cost-control requirement and reduces GPT-5 usage by caching identical generations.
...
- **Alternatives considered**: (a) Free-form prompt parsing — higher validation risk; (b) Non-GPT provider — out of scope for this v1.
...
- **Rationale**: Keeps sensitive data off the client and supports the ≤20s p95 target with transparent feedback.
- **Alternatives considered**: (a) Client-side PDF rendering — adds complexity and keeps sensitive data on the client; (b) No streaming feedback — conflicts with success criteria for perceived responsiveness.
