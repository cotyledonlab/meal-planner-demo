...
**⚠️ CRITICAL: Always work within your worktree**:

```bash
# WRONG: Committing from main repo directory
cd /home/john/meal-planner-demo  # ❌ Main repo
git commit -m "fix: something"    # ❌ Commits to wrong branch

# RIGHT: Work only in your issue-specific worktree
cd ../meal-planner-{issue-number}  # ✅ Your worktree
git commit -m "fix: something"     # ✅ Commits to correct branch
git push                           # ✅ Pushes your feature branch only
```

**Why this matters**:

- Prevents accidentally committing to unrelated branches
- Keeps work isolated and organized
- Makes it impossible to push to wrong branch
- Each worktree is locked to its own branch

**Standard workflow steps**:

1. Create feature branch in worktree
2. Make code changes **within the worktree directory**
3. Push to branch → Create/update PR **from within worktree**
